...
class MyArc : public GraphicElement{
	private:
		TPoint points[3]; // массив точек дл€ построени€ кривой Ѕезье
		TColor color;
		int bold;
...
class MyPolyline : public GraphicElement{
     ...
     TColor color;
	 int bold;
	 int a = -1; //индекс дл€ вектора элементов
...
std::vector<MyPolyline> polylines;
std::vector<MyCircle> circles;
std::vector<int> sequence; // пор€док элементов 1 - отрезокб 2 -дуга 3-полилини€
TColor color; // активный цвет
TColor color2 = clWhite; // цвет заливки
int bold = 1; // активна€ толщина
int i = -1; // текущий индекс дл€ вектора отрезков
int a = -1; // текущий индекс дл€ вектора дуг
int p = -1; // текущий индекс дл€ вектора полилиний
int e = -1; // текущий индекс дл€ вектора полилиний
int point=0; // перва€ или втора€ точка отрезка
int x,y; //начальна€ точка отрезка
...
void __fastcall TForm1::New1Click(TObject *Sender)
{
  int k;
	if(Form1->Save1->Enabled == true){
	   ...
	   k = Application->MessageBox(L"—охранить текущее изображение?", L"PlyPaint —охранение", MB_YESNOCANCEL);
	   #else
	   k = Application->MessageBox("—охранить текущее изображение?", "PlyPaint —охранение", MB_YESNOCANCEL);
	   #endif
          ...
          int Y)
{
	...
	if(SpeedButton3 -> Down == true){
	 if(point == 1){
		redrawing(segments,arcs,polylines,circles,sequence,Image1,fileName); // перерисовываем все элементы
